        ...
        if types == "Conv2d":
            stact.append(
                [
                    name,
                    types,
                    [noda.in_channels, noda.out_channels],
                    noda.kernel_size[0] * noda.kernel_size[1],
                ]
            )
...
def get_mask(model):
    ...
    for i, n in enumerate(nodes):
        if len(n.args) > 0:
            if n.op == "call_module":
                if (
                    get_type(eval(f"model.{rename(str(n.target))}")) == "Conv2d"
                    and eval(f"model.{rename(str(n.target))}").groups == 1
                    or get_type(eval(f"model.{rename(str(n.target))}")) == "Linear"
                ):
                    ...
                    while True:
                        ...
                        for j, n in enumerate(nodes):
                            if (
                                ...
                                if n.op == "call_module":
                                    if (
                                        get_type(eval(f"model.{rename(str(n.target))}"))
                                        == "Conv2d"
                                        and eval(
                                            f"model.{rename(str(n.target))}"
                                        ).groups
                                        == 1
                                    ):
                                        ...
                                        == "Conv2d"
                                        or get_type(
                                            eval(f"model.{rename(str(n.target))}")
                                        )
                                        == "BatchNorm2d"
                                    ):
                                        add(crops[-1], [rename(str(n.target)), 0])
...
def delet(model, Delet_Name_sloi, i=-1, j=-1):
    ...
    if types == "Conv2d":
        ...
        if bias != None:
            ...
            if i != -1:
                bias_w = delet_weight(
                    eval("model.{}".format(Delet_Name_sloi)).bias, i, j
                )
        ...
        track_running_stats = eval(
            "model.{}".format(Delet_Name_sloi)
        ).track_running_stats
        ...
        # Передать веса
        new_pam.weight = delet_weight(
            eval("model.{}".format(Delet_Name_sloi)).weight, i, j
        )
        ...
        # Передать веса
        new_pam.weight = delet_weight_Linear(
            eval("model.{}".format(Delet_Name_sloi)).weight, j, i
        )
        new_pam.bias = delet_weight_Linear(
            eval("model.{}".format(Delet_Name_sloi)).bias, j
        )
...
def get_param():
    ...
    parser.add_argument(
        "--orig_size", type=float, default=100, help="Размер этого слоя до обрезки"
    )
    parser.add_argument(
        "--iterr",
        type=int,
