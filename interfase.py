...
def main():
    ...
    def _radio(sender, app_data, user_data):
        pass
    ...
    def start(sender, app_data, user_data):
        ...
        config.model.size = [
            int(dpg.get_value("model.size[0]")),
            int(dpg.get_value("model.size[1]")),
        ]
        ...
        config.retraining.dataLoader.batch_size_t = dpg.get_value(
            "retraining.dataLoader.batch_size_t"
        )
        config.retraining.dataLoader.num_workers_t = dpg.get_value(
            "retraining.dataLoader.num_workers_t"
        )
        config.retraining.dataLoader.pin_memory_t = dpg.get_value(
            "retraining.dataLoader.pin_memory_t"
        )
        config.retraining.dataLoader.drop_last_t = dpg.get_value(
            "retraining.dataLoader.drop_last_t"
        )
        config.retraining.dataLoader.shuffle_t = dpg.get_value(
            "retraining.dataLoader.shuffle_t"
        )
        config.retraining.dataLoader.batch_size_v = dpg.get_value(
            "retraining.dataLoader.batch_size_v"
        )
        config.retraining.dataLoader.num_workers_v = dpg.get_value(
            "retraining.dataLoader.num_workers_v"
        )
        config.retraining.dataLoader.pin_memory_v = dpg.get_value(
            "retraining.dataLoader.pin_memory_v"
        )
        config.retraining.dataLoader.drop_last_v = dpg.get_value(
            "retraining.dataLoader.drop_last_v"
        )
        config.retraining.dataLoader.shuffle_v = dpg.get_value(
        ...
        config.my_pruning.restart.start_iteration = dpg.get_value(
            "my_pruning.restart.start_iteration"
        )
    ...
    def reset_log(sender, app_data, user_data):
        ...
        with dpg.group(tag="statistica", parent="Mstatistica"):
            df = pd.read_csv(os.path.join(exp_save, f"{model_name}_log.csv"))
            arr = df.to_numpy()
            with dpg.table():
            ...
            with dpg.tree_node(label="Метрика от степини обрезки"):
                with dpg.plot(label="Метрика от степини обрезки", height=500, width=-1):
                    ...
                    with dpg.plot_axis(dpg.mvYAxis, label="Метрика"):
                        dpg.add_line_series(
                            list(1 - df["size"].values),
                            list(df["acc"].values),
                            label="Метрика от степини обрезки",
                        )
            with dpg.tree_node(label=f"Данные по итерациям", default_open=False):
                ...
                for i in ind:
                    with dpg.tree_node(label=f"Итерация {i}", default_open=False):
                        df = pd.read_csv(
                            os.path.join(
                                exp_save, model_name, f"{model_name}_it{i}.csv"
                            )
                        )
                        arr = df.to_numpy()
                        with dpg.table():
    ...
    def graf(sender, app_data, user_data):
        ...
        node_specs = [
            [n.op, n.name, str(n.target), n.args, n.kwargs] for n in gm.graph.nodes
        ]
    ...
    def onnx():
        ...
        with dpg.group(tag="onnx", parent="Monnx"):
            for mame in inport:
                ...
                save = os.path.join(
                    export, os.path.basename(mame).split(".pth")[0] + ".onnx"
                )
                torch.onnx.export(
                    model,
                    dummy_input,
    ...
    with dpg.window(tag="main", menubar=True):
        ...
        with dpg.menu_bar():
            with dpg.menu(label="Tools"):
                dpg.add_menu_item(
                    label="Показать меню стилей",
                    callback=lambda: dpg.show_tool(dpg.mvTool_Style),
                )
                dpg.add_menu_item(
                    label="Показать меню шрифтов",
                    callback=lambda: dpg.show_tool(dpg.mvTool_Font),
                )
            with dpg.menu(label="Settings"):
                dpg.add_menu_item(
                    label="Обработка во время импута",
                    check=True,
        ...
        with dpg.tab_bar(tag="tab_bar"):
            with dpg.tab(label="Прунинг"):
                with dpg.tree_node(
                    label="Основные параметры", default_open=True, tag="tree_node_1"
                ):
                    with dpg.group(horizontal=True):
                        with dpg.group():
                        ...
                        with dpg.group():
                            ...
                            with dpg.group(horizontal=True):
                                ...
                                dpg.add_button(
                                    ...
                                    user_data="path.exp_save",
                                )
                            dpg.add_input_text(
                                default_value="['timm.layers.norm_act']",
                                width=len_input_text,
                                callback=_log,
                                no_spaces=True,
                                tag="class_name",
                ...
                with dpg.tree_node(
                    label="Модель", default_open=True, tag="tree_node_2"
                ):
                    with dpg.group(horizontal=True):
                        with dpg.group():
                        ...
                        with dpg.group():
                            ...
                            with dpg.group(horizontal=True):
                                ...
                                help_i += 1
                with dpg.tree_node(
                    label="Набор данных", default_open=True, tag="tree_node_3"
                ):
                    with dpg.group(horizontal=True):
                        with dpg.group():
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            with dpg.group():
                                dpg.add_input_int(
                                    ...
                                    min_value=1,
                                    min_clamped=True,
                                    tag="training.dataLoader.batch_size_t",
                                )
                                dpg.add_input_int(
                                    callback=_log,
                                    width=len_input_int,
                                    ...
                                    min_value=0,
                                    min_clamped=True,
                                    tag="training.dataLoader.num_workers_t",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="training.dataLoader.pin_memory_t",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="training.dataLoader.drop_last_t",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="training.dataLoader.shuffle_t",
                                )
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            with dpg.group():
                                dpg.add_input_int(
                                    ...
                                    min_value=1,
                                    min_clamped=True,
                                    tag="training.dataLoader.batch_size_v",
                                )
                                dpg.add_input_int(
                                    callback=_log,
                                    width=len_input_int,
                                    ...
                                    min_value=0,
                                    min_clamped=True,
                                    tag="training.dataLoader.num_workers_v",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="training.dataLoader.pin_memory_v",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="training.dataLoader.drop_last_v",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="training.dataLoader.shuffle_v",
                                )
                with dpg.tree_node(
                    label="Конфигурация востановления",
                    default_open=False,
                    tag="tree_node_5",
                ):
                    with dpg.group(horizontal=True):
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            with dpg.group():
                                dpg.add_input_int(
                                    ...
                                    min_value=1,
                                    min_clamped=True,
                                    tag="retraining.dataLoader.batch_size_t",
                                )
                                dpg.add_input_int(
                                    callback=_log,
                                    width=len_input_int,
                                    ...
                                    min_value=0,
                                    min_clamped=True,
                                    tag="retraining.dataLoader.num_workers_t",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="retraining.dataLoader.pin_memory_t",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="retraining.dataLoader.drop_last_t",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="retraining.dataLoader.shuffle_t",
                                )
                    with dpg.tree_node(label="валидация", default_open=False):
                        with dpg.group(horizontal=True):
                            with dpg.group():
                            ...
                            with dpg.group():
                                dpg.add_input_int(
                                    ...
                                    min_value=1,
                                    min_clamped=True,
                                    tag="retraining.dataLoader.batch_size_v",
                                )
                                dpg.add_input_int(
                                    callback=_log,
                                    width=len_input_int,
                                    ...
                                    min_value=0,
                                    min_clamped=True,
                                    tag="retraining.dataLoader.num_workers_v",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="retraining.dataLoader.pin_memory_v",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="retraining.dataLoader.drop_last_v",
                                )
                                dpg.add_checkbox(
                                    callback=_log,
                                    default_value=True,
                                    tag="retraining.dataLoader.shuffle_v",
                                )
                with dpg.tree_node(
                    label="Параметры алгоритма", default_open=False, tag="tree_node_6"
                ):
                    with dpg.group(horizontal=True):
                        with dpg.group():
                        ...
                        with dpg.group():
                            ...
                            help_i += 1
                            dpg.add_radio_button(
                                ("TaylorFOWeight", "L2Norm"),
                                callback=_radio,
                                horizontal=True,
                                tag="my_pruning.algoritm",
                                default_value="TaylorFOWeight",
                            )
                            _help(helps[help_i])
                            help_i += 1
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            with dpg.group():
                                dpg.add_input_int(
                                    ...
                                    min_value=0,
                                    min_clamped=True,
                                    tag="my_pruning.restart.start_iteration",
                                )
                                with dpg.group(horizontal=True):
                                    dpg.add_input_text(
                                        default_value=os.path.join(
                                        ...
                                        callback=_log,
                                        no_spaces=True,
                                        tag="my_pruning.restart.load",
                                    )
                                    dpg.add_button(
                                        label="Найти",
                                        callback=select_file,
                                        user_data="my_pruning.restart.load",
                                    )
                with dpg.table(
                    tag="Goods",
                    header_row=False,
                    ...
                    with dpg.group():
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(
                                ...
                                callback=_log,
                                no_spaces=True,
                                tag="debag.path.exp_save",
                            )
                            dpg.add_button(
                                label="Найти",
                                callback=select_file,
                                user_data="debag.path.exp_save",
                            )
                        dpg.add_input_text(
                            default_value="['timm.layers.norm_act']",
                            width=len_input_text,
                            callback=_log,
                            no_spaces=True,
                            tag="debag.class_name",
                        )
                        _help(helps[1])
                        dpg.add_input_text(
                            default_value="[1,3,224,224]",
                            width=len_input_text,
                            callback=_log,
                            no_spaces=True,
                            tag="debag.shape",
                    ...
                    with dpg.group():
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(
                                ...
                                callback=_log,
                                no_spaces=True,
                                tag="onnx.path.inport",
                            )
                            dpg.add_button(
                                label="Найти",
                                callback=select_files,
                                user_data="onnx.path.inport",
                            )
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(
                                width=len_input_text,
                                callback=_log,
                                no_spaces=True,
                                tag="onnx.path.export",
                            )
                            dpg.add_button(
                                label="Найти",
                                callback=select,
                                user_data="onnx.path.export",
                            )
                        dpg.add_input_text(
                            default_value="[1,3,224,224]",
                            width=len_input_text,
