...
if task_tupe != "detection":
    ...
    ttrain_dataloader = DataLoader(trainset, batch_size=tbatch_size_t, num_workers=tnum_workers_t, pin_memory=tpin_memory_t, drop_last=tdrop_last_t, shuffle=tshuffle_t)
    tval_dataloader = DataLoader(valset, batch_size=tbatch_size_v, num_workers=tnum_workers_v, pin_memory=tpin_memory_v, drop_last=tdrop_last_v, shuffle=tshuffle_v)
else:
    train_transform, val_transform = detect.get_ransforms(size[0],size[1])
    val_dataset = detect.MyDataset(os.path.join(annotation_path, annotation_val),
                        annotation_path,
                        "val",
                        val_transform)
    train_dataset = detect.MyDataset(os.path.join(annotation_path, annotation_train),
                        annotation_path,
                        "train",
                        train_transform)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size_t, num_workers=num_workers_t, pin_memory=pin_memory_t, drop_last=drop_last_t,shuffle=shuffle_t, collate_fn = detect.custom_collate)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size_v, num_workers=num_workers_v, pin_memory=pin_memory_v, drop_last=drop_last_v,shuffle=shuffle_v, collate_fn = detect.custom_collate)
    ttrain_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=tbatch_size_t, num_workers=tnum_workers_t, pin_memory=tpin_memory_t, drop_last=tdrop_last_t,shuffle=tshuffle_t, collate_fn = detect.custom_collate)
    tval_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=tbatch_size_v, num_workers=tnum_workers_v, pin_memory=tpin_memory_v, drop_last=tdrop_last_v,shuffle=tshuffle_v, collate_fn = detect.custom_collate)
